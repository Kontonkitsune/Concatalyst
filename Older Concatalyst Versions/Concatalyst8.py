	...
	for x in range(0,43):
		for y in range(0,5):
			if ConcatalystTitleBoard[x,y].color:
				...
				if ConcatalystConnectedBoard[x,y]:
			...
			for z in range(1,len(extrablocks)):
				for x in range(0,2):
					for y in range(0,2):
						...
						draw_square(extrablocks[z][x,y],x - 3, 17 + y - (3 * z),connected=False)
...
def draw_square(square, x: int,y: int, blocksize = 0, connected = True) -> None:
	...
	if connectionType.value == 0 and (special == 1 or connected and bombtimer > 0 and ConnectedBoard[x,y] > 0):
	...
	if special == 1 or (connected and bombtimer > 0 and ConnectedBoard[x,y] > 0):
		if connectionType.value == 0:
			if special == 1:
					pygame.draw.circle(window,get_square_color(square), (squaretopleftx + blocksize // 2, squaretoplefty + blocksize // 2 ), blocksize // 2 - 5)
		elif connectionType.value == 1:
			pygame.draw.line(window,BLACK,
				(squaretopleftx + blocksize // 2,	squaretoplefty),
				(squaretopleftx + blocksize // 2,	squaretoplefty + blocksize),		3)
			pygame.draw.line(window,BLACK,
				(squaretopleftx,					squaretoplefty + blocksize // 2),
				(squaretopleftx + blocksize,		squaretoplefty + blocksize // 2),	3)
			if special == 1:
			...
			for p in ((1,0),(-1,0),(0,1),(0,-1)):
				if isinbounds(GameBoard,x + p[0],y + p[1]) and ConnectedBoard[x + p[0],y + p[1]] != 0 and ConnectedBoard[x,y] != 0 and abs(ConnectedBoard[x + p[0],y + p[1]] - ConnectedBoard[x,y]) == 1:
					pygame.draw.line(window,BLACK,	(squaretopleftx + blocksize // 2,	squaretoplefty + blocksize // 2),
															(squaretopleftx + blocksize // 2 + p[0] * blocksize,	squaretoplefty + blocksize // 2 - p[1] * blocksize),3)
