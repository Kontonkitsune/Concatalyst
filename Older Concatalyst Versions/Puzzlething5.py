...
GameBoard = Board(BOARD_SIZE_X,BOARD_SIZE_Y + 4) # The game board. It's a bit bigger than it looks.
...
def init_board() -> None: # Initializes the board
	...
	for x in range(0,MAX_QUEUE_LENGTH):
		...
		if len(extrablocks) > MAX_QUEUE_LENGTH:
			...
			break
	reinitialize()
	ConnectedBoard = Board(boardWidth.value,boardHeight.value)
	if game_state:
		GameBoard = Board(boardWidth.value,boardHeight.value)
	else:
		bombtimer = catalystTimer.value
		GameBoard = Board(boardWidth.value,boardHeight.value)
		if random.randint(1,10) == 1 and GameBoard.width == 10 and GameBoard.height == 20:
			for x in range(0,10):
				for y in range(0,16):
					GameBoard[x,y] = literallythebestboard[y][x]
		else:
			for x in range(0,GameBoard.width):
				...
				for y in range(GameBoard.height - 4,GameBoard.height):
					if (random.randint(1,2) == 2):
						GameBoard[x,y] = generate_tile(catalystchance=True)
	ExplodeBoard = Board(boardWidth.value,boardHeight.value)
...
def findclears(localInputBoard = GameBoard) -> Board: # Searches through the board and figures out which blocks are connected to catalysts.
	...
	"""
	OutputBoard = Board(GameBoard.width,GameBoard.height)
	stack = []
