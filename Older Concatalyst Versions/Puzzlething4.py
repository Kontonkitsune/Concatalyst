...
GameBoard = Board(board_size_x,board_size_y) # The game board. It's a bit bigger than it looks.
ExplodeBoard = Board(board_size_x,board_size_y) # The board used for tracking which blocks are supposed to explode. Also acts as a handy graphics thing.
ConnectedBoard = Board(board_size_x,board_size_y)
...
goldenChance = Config(10,100,1,1)
...
def generate_tile(tiletype=0,catalystchance=False,catalyst=False) -> int: # Generates a tile value
	"""
	...
	"""
	if tiletype == 0:
		if random.randint(0,100) < goldenChance.value:
			tiletype = 7
		else:
...
def reinitialize() -> None: # Initializes variables to config values
	...
	global board_size_x, board_size_y, boardwidth, boardheight, leftscreen, rightscreen, topscreen, bottomscreen, grace_period, blockssincelastcatalyst
	...
	boardwidth = (blockSize.value * board_size_x)
	boardheight = (blockSize.value * ( board_size_y - 4) )
...
def init_board() -> None: # Initializes the board
	...
	ConnectedBoard = Board(board_size_x,board_size_y)
	if game_state:
		GameBoard = Board(board_size_x,board_size_y)
	else:
		...
		for x in range(0,GameBoard.width):
			...
			for y in range(GameBoard.height - 4,GameBoard.height):
				if (random.randint(1,2) == 2):
					GameBoard[x,y] = generate_tile(catalystchance=True)
	ExplodeBoard = Board(board_size_x,board_size_y)
...
def comparesquares(x: int, y: int, x2: int, y2: int) -> bool: # Checks if two squares are considered "connected" and can be cleared together
	if isinbounds(x,y) and isinbounds(x2,y2):
		if GameBoard[x,y] == 0 or GameBoard[x2,y2] == 0:
			return False
		elif GameBoard[x,y] % 7 == GameBoard[x2,y2] % 7 or GameBoard[x,y] % 7 == 0 or GameBoard[x2,y2] % 7 == 0:
			return True
		else:
			return False
	else:
...
def findclears(localInputBoard = GameBoard) -> Board: # Searches through the board and figures out which blocks are connected to catalysts.
	"""
	Searches through the board and returns a board of all cells connected to catalysts.
	
	
	"""
	OutputBoard = Board(board_size_x,board_size_y)
	stack = []
	
	for x in range(0,localInputBoard.width):
		for y in range(0,localInputBoard.height):
			if localInputBoard[x,y] > 7 and localInputBoard[x,y] < 15:
				OutputBoard[x,y] += 1
				stack.append((x,y))
	while len(stack) > 0:
		x = stack[0][0]
		...
		for p in ((x+1,y),(x-1,y),(x,y+1),(x,y-1)):
			if isinbounds(p[0],p[1]) and localInputBoard[p[0],p[1]] < 8 and (OutputBoard[p[0],p[1]] == 0 or OutputBoard[p[0],p[1]] > OutputBoard[x,y] + 1) and comparesquares(x,y,p[0],p[1]):
				stack.append((p[0],p[1]))
				OutputBoard[p[0],p[1]] = OutputBoard[x,y] + 1
	return OutputBoard

def catalystclearblocks(localGameBoard = GameBoard, localExplodeBoard = ExplodeBoard) -> tuple: # Deletes squares connected to catalysts and returns point values
	"""
...
def cascade_board(hard_cascade = False) -> None:
	...
	for x in range(0,GameBoard.width):
		...
		for y in range(0,GameBoard.height):
			if gravityflag and GameBoard[x,y] != 0:
				cascadeflag = True
			elif GameBoard[x,y] == 0:
				gravityflag = True
		...
		if cascadeflag:
			...
			if hard_cascade:
				for y in range(lastsafe,GameBoard.height):
					if GameBoard[x,y] != 0:
						GameBoard[x,lastsafe] = GameBoard[x,y]
						GameBoard[x,y] = 0
						lastsafe += 1
			else:
				for y in range(lastsafe,GameBoard.height):
...
def drawtile(blockID: int,x: int,y: int,usecolorinstead=False) -> None:
	
	if usecolorinstead:
		pygame.draw.rect(window,blockID,
			pygame.Rect((	
				(leftscreen + (blockSize.value * x), bottomscreen - (blockSize.value * y)	),
				(blockSize.value,blockSize.value))))
	else:
		blockcolor = ColorIndex[blockID]
		
		catalystcoloroffset = 8 * ( abs(	(1 * bombtimer + 8 * ConnectedBoard[x,y-1]) % 60 - 30) - 15 ) 
		...
		if blockID == 7 or blockID == 14:
			...
			pygame.draw.rect(window,rainbowcolor,pygame.Rect((	
						(leftscreen + (blockSize.value * x) + 5, bottomscreen - (blockSize.value * y) + 5	),
						(blockSize.value - 10,blockSize.value - 10))))
		if blockID > 7:
			pygame.draw.circle(window,ColorIndex[blockID-7], (leftscreen + (blockSize.value * x) + blockSize.value // 2, bottomscreen - (blockSize.value * y) + blockSize.value // 2 ), blockSize.value // 2 - 5)
...
def create_block(type="random",color=0,catalyst=False) -> Block:
		...
		colorsets = [generate_tile() for x in range(0,5)]
...
def explodethings() -> None: # This function is kind of unnecessary and should probably be moved to be part of findclears()
	...
	ExplodeBoard = findclears(GameBoard)
	previousscores.insert(0,tuple(catalystclearblocks(GameBoard,ExplodeBoard) + tuple([clock])))
...
def game_processing() -> None: # Ingame event handling
	...
	global extrablocks, keydown, block_pos_x, block_pos_y, block_fall_progress, clock, ExplodeBoard, score, prevscore, timesinceexplode, game_state, grace, bombtimer, ConnectedBoard
	...
	if bombtimer > 0 and bombtimer % 10 == 0:
		ConnectedBoard = findclears(GameBoard)
	...
	if grace <= 0:
		...
		if block_fall_progress > 1000:
			if block_pos_y < 1 or block_pos_y > 17 or block_pos_x < 0 or block_pos_x >= board_size_x - 1 or GameBoard[block_pos_x,block_pos_y - 2] == 0 and GameBoard[block_pos_x + 1,block_pos_y - 2] == 0 and block_pos_y >= 2:
				block_pos_y -= 1
			else:
				place_block()
...
def draw_game(explosion=False,fakeboard=False) -> None:
		...
		for y in range(0,(board_size_y - 4) ):
			if GameBoard[x,y] == 0:
				if ExplodeBoard[x,y] != 0:
					...
					if sum(excolor) > 40:
						drawtile(excolor,x,y+1,True)
			else:
				drawtile(GameBoard[x,y],x,y+1)
	...
	if not fakeboard:
		...
		if len(extrablocks) > 1:
			for z in range(1,len(extrablocks)):
				for x in range(0,2):
					for y in range(0,2):
						drawtile(extrablocks[z][x,y],x - 3, 18 + y - (3 * z) )
		
		#Draw the active block
		for x in range(0,2):
			for y in range(0,2):
				drawtile(extrablocks[0][x,y],	(x + block_pos_x),	(y + block_pos_y))
	...
	if bombtimer > 0:
		bombline = (board_size_y - 4) * blockSize.value * bombtimer // bombTimer.value
		for x in range(0,board_size_x + 1 ):
...
def draw_menu() -> None: # Draws the menu
	...
	if optionsmenu:
		...
		menutext(f"Gold Chance: {goldenChance.value}",goldenChance)
		menutext(f"Colors: {numberColors.value}",numberColors)
		menutext(f"Test: {blockssincelastcatalyst}")
